...
print("="*70, flush=True)
print("神经网络训练 - 600股扩展版", flush=True)
print("="*70, flush=True)
...
files = sorted(HISTORY_DIR.glob("*.json"))[:600]
...
for p in [MODEL_DIR / f"ml_nn_600_{ts}.pkl", MODEL_DIR / "ml_nn_production.pkl"]:
    with open(p, 'wb') as f: pickle.dump(model_data, f)

print(f"\n保存: ml_nn_600_{ts}.pkl", flush=True)
print("="*70, flush=True)