...
print(f"\n数据加载:", flush=True)
print(f"  股票: {stocks_ok}, 样本: {len(X_all)}, 2024:{samples_2024}, 2025:{samples_2025}, 2026:{samples_2026}", flush=True)
print(f"  正样本: {sum(y_all)/len(y_all)*100:.2f}%, 耗时: {time.time()-start:.1f}s", flush=True)
...
print(f"\n训练: {len(X_train)}, 测试: {len(X_test)}", flush=True)
t0 = time.time()

model = MLPClassifier(hidden_layer_sizes=(128, 64), activation='relu', solver='adam',
                      learning_rate_init=0.008, max_iter=50, early_stopping=True,
                      validation_fraction=0.1, n_iter_no_change=6, random_state=42, alpha=0.01)
