...
test_df['trade_date'] = pd.to_datetime(test_df['trade_date'], format='%Y%m%d')
test_df = test_df.sort_values(['code', 'trade_date']).reset_index(drop=True)
...
# 按股票分组计算特征
test_df = test_df.groupby('code', group_keys=False).apply(compute_features)
...
for model_file in sorted(model_files):
    ...
    try:
        ...
        # 排除最后forward天（按股票分组）
        valid_df = valid_df.groupby('code', group_keys=False).apply(
