...
print("="*70, flush=True)
print("神经网络全量训练 - 超深网络版本", flush=True)
print("="*70, flush=True)

files = sorted(HISTORY_DIR.glob("*.json"))
total_files = len(files)
print(f"总股票数: {total_files}", flush=True)
print(f"超深网络: 128-64-32, 学习率0.005, 每批迭代10次", flush=True)

# ✅ 超深网络 + 更低学习率 + 更多迭代
model = MLPClassifier(
    hidden_layer_sizes=(128, 64, 32),  # ✅ 超深网络
    activation='relu',
    solver='adam',
...
model_data = {
    ...
    'metadata': {
        'auc': auc, 'version': 'V10_全量超深网络',
        'total_stocks': total_stocks, 'total_samples': total_samples,
        'test_samples': len(test_y_all), 'total_time': total_time,
        'batches': batch_results, 'network': '128-64-32',
        'learning_rate': 0.005, 'max_iter': 10
    }
}

for p in [MODEL_DIR / f"ml_nn_full_deep_{ts}.pkl", MODEL_DIR / "ml_nn_production.pkl"]:
    with open(p, 'wb') as f: pickle.dump(model_data, f)

print(f"\n保存: ml_nn_full_deep_{ts}.pkl", flush=True)
print("="*70, flush=True)