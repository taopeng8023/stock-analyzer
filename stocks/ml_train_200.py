...
print("="*70, flush=True)
print("神经网络训练 - 200股增强版", flush=True)
print("="*70, flush=True)
...
print(f"  加载耗时: {time.time()-start:.1f}s", flush=True)
...
print(f"训练耗时: {time.time()-t0:.1f}s", flush=True)
...
for t in [0.5, 0.6, 0.7, 0.8]:
    ...
    count = sum(pred)
    print(f"  确率@{t}: {prec:.4f} ({count}股)", flush=True)
...
model_data = {
    ...
    'metadata': {
        'auc': auc,
        'precision_50': precision_score(y_test, (proba >= 0.5).astype(int), zero_division=0),
        'precision_60': precision_score(y_test, (proba >= 0.6).astype(int), zero_division=0),
        'precision_70': precision_score(y_test, (proba >= 0.7).astype(int), zero_division=0),
        'precision_80': precision_score(y_test, (proba >= 0.8).astype(int), zero_division=0),
        'version': 'V10_200股_无ret5',
...
for p in [MODEL_DIR / f"ml_nn_200_{ts}.pkl", MODEL_DIR / "ml_nn_production.pkl"]:
    with open(p, 'wb') as f: pickle.dump(model_data, f)

print(f"\n模型已保存:", flush=True)
print(f"  {MODEL_DIR}/ml_nn_200_{ts}.pkl", flush=True)
print(f"  {MODEL_DIR}/ml_nn_production.pkl (生产模型)", flush=True)
print("="*70, flush=True)
