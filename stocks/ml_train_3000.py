...
print("="*70, flush=True)
print("神经网络训练 - 3000股超优化版", flush=True)
print("="*70, flush=True)
...
files = sorted(HISTORY_DIR.glob("*.json"))[:3000]
...
model_data = {
    ...
    'metadata': {'auc': auc, 'version': 'V10_3000股_超优', 'stocks': stocks_ok,
                 'train_samples': len(X_train), 'load_time': load_time, 'train_time': train_time}
}

for p in [MODEL_DIR / f"ml_nn_3000_{ts}.pkl", MODEL_DIR / "ml_nn_production.pkl"]:
    with open(p, 'wb') as f: pickle.dump(model_data, f)

print(f"\n保存: ml_nn_3000_{ts}.pkl", flush=True)
print("="*70, flush=True)