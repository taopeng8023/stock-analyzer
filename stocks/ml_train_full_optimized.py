...
"""
全量训练 - 优化参数版本
2.9百万样本，网络64-32，学习率0.01，每批迭代5次
"""
...
print("="*70, flush=True)
print("神经网络全量训练 - 优化参数版本", flush=True)
print("="*70, flush=True)

files = sorted(HISTORY_DIR.glob("*.json"))
total_files = len(files)
print(f"总股票数: {total_files}", flush=True)
print(f"优化参数: 网络64-32, 学习率0.01, 每批迭代5次", flush=True)

# ✅ 优化后的模型参数
model = MLPClassifier(
    hidden_layer_sizes=(64, 32),  # ✅ 增大网络
    activation='relu',
    solver='adam',
    learning_rate_init=0.01,  # ✅ 降低学习率
    max_iter=5,  # ✅ 每批迭代5次
    warm_start=True,  # 增量训练
    random_state=42,
    alpha=0.001,  # ✅ 降低正则化
    early_stopping=False  # ✅ 关闭早停，让模型充分训练
)
...
model_data = {
    ...
    'metadata': {
        ...
        'learning_rate': 0.01, 'max_iter': 5
    }
}

for p in [MODEL_DIR / f"ml_nn_full_optimized_{ts}.pkl", MODEL_DIR / "ml_nn_production.pkl"]:
    with open(p, 'wb') as f: pickle.dump(model_data, f)

print(f"\n保存: ml_nn_full_optimized_{ts}.pkl", flush=True)
print("="*70, flush=True)