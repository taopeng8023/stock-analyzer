...
LOG_FILE = Path('/Users/taopeng/.openclaw/workspace/stocks/ml_train_neural_v6_continue.log')
...
def prepare_data():
    ...
    n_stocks = min(1000, len(stock_files))  # 减少到 1000 只
    ...
    n_pos = min(len(positives), 170000)  # 减少样本
    n_neg = min(len(negatives), 330000)
    ...
    # 保存临时数据
    temp_dir = MODEL_DIR / 'temp_v6_small'
...
def train_single_model(temp_dir, seed, model_idx):
    ...
    # 保存单个模型
    model_path = MODEL_DIR / f'ml_nn_v6_model{model_idx+1}_seed{seed}.pkl'
...
def evaluate_ensemble(temp_dir, model_paths):
    ...
    config_path = MODEL_DIR / f'ml_nn_v6_ensemble_config_{timestamp}.json'
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f'训练日志 V6 继续版 - {datetime.now()}\n\n')
    
    log('='*60)
    log('神经网络选股模型 V6 - 继续训练模型 3-5 (小规模版)')
    log('='*60)
    ...
    seeds = [456, 789, 1024]  # 只训练 3 个模型
    model_paths = []
    
    # 检查已有模型
    for i in [1, 2]:
        existing_seed = [42, 123][i-1]
        existing_path = MODEL_DIR / f'ml_nn_v6_model{i}_seed{existing_seed}.pkl'
        if existing_path.exists():
            log(f'✅ 模型{i} 已存在：{existing_path.name}')
            model_paths.append(existing_path)
    
    for i, seed in enumerate(seeds):
        model_idx = i + 3  # 模型 3/4/5
        model_path = MODEL_DIR / f'ml_nn_v6_model{model_idx}_seed{seed}.pkl'
        model_paths.append(model_path)
        
        auc = train_single_model(temp_dir, seed, model_idx-1)
        log(f'模型 {model_idx}/5 完成，AUC={auc:.4f}')
        gc.collect()
