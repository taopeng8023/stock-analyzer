...
LOG_FILE = Path('/Users/taopeng/.openclaw/workspace/stocks/ml_train_neural_v6_batch.log')
...
def prepare_data():
    ...
    n_stocks = min(1500, len(stock_files))  # 减少到 1500 只
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f'训练日志 V6 分批版 - {datetime.now()}\n\n')
    
    log('='*60)
    log('神经网络选股模型 V6 - 分批训练版')
    log('='*60)
    ...
    for i, seed in enumerate(seeds):
        model_path = MODEL_DIR / f'ml_nn_v6_model{i+1}_seed{seed}.pkl'
