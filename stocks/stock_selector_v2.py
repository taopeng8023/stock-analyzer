#!/usr/bin/env python3
"""
双均线选股器 v3.3 - 使用最优参数 MA15/MA20 + 移动止盈 10%

更新说明 (2026-03-28):
- v3.0: 基于 5493 只股票全市场回测优化
- v3.1: 新增止损机制 (固定 -15%, 移动 -20%)
- v3.2: 新增成交量卖出策略 (回测失效，已禁用)
- v3.3: 新增移动止盈 10% (基于最优卖出分析优化)
- 最优参数：MA15/MA20 (平均收益 +20-23%, 胜率 50-67%)
- 移动止盈：从最高点回撤 10% 卖出 (收益捕获率 60-80%)
...
用法:
    ...
    python3 stock_selector_v2.py --backtest 600152  # 回测单只股票
"""
...
class EnhancedStockSelector:
    """增强版选股器 v3.3 - 使用最优参数 MA15/MA20 + 移动止盈 10%"""
    
    # 最优均线参数 (基于 5493 只股票回测优化 - 2026-03-28)
    # 回测结果：MA15/MA20 平均收益 +20.22%, 胜率 48.5%, 盈利股票 62.3%
    MA_SHORT = 15  # 短期均线 (最优)
    MA_LONG = 20   # 长期均线 (最优)
    ...
    STOP_LOSS_CONFIG = {
        ...
        'trailing_stop': 0.20,        # 移动止盈 -20% (从最高点回撤)
        'enable_stop_loss': True,     # 启用止损
    }
    
    # 成交量卖出配置 (v3.2 新增 - 基于全市场回测优化)
    VOLUME_EXIT_CONFIG = {
        'enable_volume_exit': False,  # 禁用成交量卖出 (回测失效)
        'volume_shrink_threshold': 0.5,  # 成交量萎缩 <0.5 倍
        'volume_spike_threshold': 2.0,   # 成交量放大 >2 倍
        'exit_strategy': 'trailing_stop',  # 使用移动止盈
    }
    
    # 移动止盈配置 (v3.3 新增 - 基于最优卖出分析优化)
    TRAILING_STOP_CONFIG = {
        'enable_trailing_stop': True,  # 启用移动止盈
        'trailing_stop_pct': 0.10,     # 从最高点回撤 10% 卖出 (最优)
    }
    ...
    def check_golden_cross(self, data: List[Dict]) -> Dict:
        ...
        # 1. 基础金叉 (使用最优参数 MA15/MA20)
        ma_short = self.calc_ma(data, self.MA_SHORT, idx)
        ma_long = self.calc_ma(data, self.MA_LONG, idx)
        ma_short_prev = self.calc_ma(data, self.MA_SHORT, idx-1)
        ma_long_prev = self.calc_ma(data, self.MA_LONG, idx-1)
        ...
        ma_short_prev_n = self.calc_ma(data, self.MA_SHORT, idx-slope_days)
        ma_long_prev_n = self.calc_ma(data, self.MA_LONG, idx-slope_days)
        if ma_short_prev_n and ma_long_prev_n:
            if ma_short > ma_short_prev_n and ma_long > ma_long_prev_n:
    ...
    def backtest_with_stoploss(self, data: List[Dict]) -> Dict:
        """
        带止损的回测 (v3.1 新增)
        ...
        for i in range(self.MA_LONG, len(data)):
            ma_short = self.calc_ma(data, self.MA_SHORT, i)
            ma_long = self.calc_ma(data, self.MA_LONG, i)
            ma_short_prev = self.calc_ma(data, self.MA_SHORT, i-1)
            ma_long_prev = self.calc_ma(data, self.MA_LONG, i-1)
            
            if None in (ma_short, ma_long, ma_short_prev, ma_long_prev):
                continue
            ...
            if not holding:
                ...
                # 1. 移动止盈 10% (v3.3 新增 - 基于最优卖出分析优化)
                if self.TRAILING_STOP_CONFIG['enable_trailing_stop']:
                    if highest_price > buy_price:
                        trailing_stop = highest_price * (1 - self.TRAILING_STOP_CONFIG['trailing_stop_pct'])
                        if current_price < trailing_stop:
                            sell_signal = True
                            sell_reason = f'移动止盈 ({(current_price-highest_price)/highest_price*100:.1f}%)'
                
                # 2. 固定止损 -15%
                if not sell_signal and self.STOP_LOSS_CONFIG['enable_stop_loss']:
                    if (current_price - buy_price) / buy_price < -self.STOP_LOSS_CONFIG['fixed_stop_loss']:
                        sell_signal = True
                        sell_reason = f'止损 ({(current_price-buy_price)/buy_price*100:.1f}%)'
                
                # 3. 死叉卖出 (v3.0 基准)
                if not sell_signal and ma_short_prev >= ma_long_prev and ma_short < ma_long:
                    sell_signal = True
                    sell_reason = '死叉'
                
                # 4. 成交量卖出 (v3.2 - 已禁用，回测失效)
                if not sell_signal and self.VOLUME_EXIT_CONFIG['enable_volume_exit']:
                    volume = data[i].get('成交量', data[i].get('volume'))
                    avg_volume = sum(data[j].get('成交量', data[j].get('volume')) for j in range(i-20, i)) / 20
                    
                    if avg_volume > 0:
                        volume_ratio = volume / avg_volume
                        
                        if self.VOLUME_EXIT_CONFIG['exit_strategy'] == 'volume_shrink':
                            if volume_ratio < self.VOLUME_EXIT_CONFIG['volume_shrink_threshold']:
                                sell_signal = True
                                sell_reason = f'成交量萎缩 ({volume_ratio:.2f}倍)'
                        elif self.VOLUME_EXIT_CONFIG['exit_strategy'] == 'volume_spike':
                            if volume_ratio > self.VOLUME_EXIT_CONFIG['volume_spike_threshold']:
                                sell_signal = True
                                sell_reason = f'成交量放大 ({volume_ratio:.2f}倍)'
