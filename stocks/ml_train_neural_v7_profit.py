...
优化方向:
- 标签优化：从"涨超 3%"改为"涨超 5%"（更高收益目标）
- 收益加权：给高收益样本更高训练权重
- 特征增强：增加收益相关特征（涨跌幅度、收益分布）
- 风险过滤：排除高风险股票（波动率过高）
- 保持 5 模型集成架构
"""
...
LOG_FILE = Path('/Users/taopeng/.openclaw/workspace/stocks/ml_train_neural_v7_profit.log')
...
def load_and_prepare_stock(stock_file, min_records=150):
    ...
    try:
        ...
        # 2. 收益风险比
        df['return_vol_ratio'] = df['future_return_5d'] / (df['volatility5'] + 0.01)
        ...
        df['avg_up_day'] = df['pct_chg'].where(df['pct_chg'] > 0, 0).rolling(10).mean()
        df['avg_down_day'] = df['pct_chg'].where(df['pct_chg'] < 0, 0).rolling(10).mean()
        ...
        # 样本权重：收益越高的样本权重越大
        df['sample_weight'] = np.clip(df['future_return_5d'] * 10 + 1, 0.1, 5.0)
        ...
        df = df.dropna(subset=feature_cols + ['label', 'sample_weight'])
        
        if len(df) < 80:
            return None
        
        return df[feature_cols + ['label', 'sample_weight']]
...
def prepare_data():
    ...
    weights = sampled['sample_weight'].values
    
    log(f'特征矩阵：{X.shape}')
    log(f'特征内存：{X.nbytes / 1024**2:.1f}MB')
    log(f'样本权重范围：{weights.min():.2f} - {weights.max():.2f}')
    ...
    X_train, X_val, y_train, y_val, w_train, w_val = train_test_split(
        X_train_full, y_train_full, w_train_full, test_size=0.15, random_state=42, stratify=y_train_full
    )
    ...
    np.save(temp_dir / 'weights_train.npy', w_train)
    ...
    del X, y, weights, X_train_s, X_val_s, X_test_s, combined, all_dfs, sampled
    gc.collect()
    
    return temp_dir

def train_single_model(temp_dir, seed, model_idx):
    """训练单个模型"""
    log(f'\n加载数据...')
    ...
    log(f'训练模型 {model_idx+1} (seed={seed})...')
    ...
    # 保存单个模型
    model_path = MODEL_DIR / f'ml_nn_v7_model{model_idx+1}_seed{seed}.pkl'
...
def evaluate_ensemble(temp_dir, model_paths):
    """评估集成模型"""
    log('\n' + '='*60)
    log('评估集成模型...')
    log('='*60)
    ...
    ensemble_config = {
        ...
        'timestamp': timestamp,
        'label_threshold': 0.05,  # V7 使用 5% 收益阈值
        'version': 'v7_profit'
    }
    
    config_path = MODEL_DIR / f'ml_nn_v7_ensemble_config_{timestamp}.json'
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f'训练日志 V7 - {datetime.now()}\n\n')
    ...
    for i, seed in enumerate(seeds):
        model_path = MODEL_DIR / f'ml_nn_v7_model{i+1}_seed{seed}.pkl'
        model_paths.append(model_path)
        
        auc = train_single_model(temp_dir, seed, i)
        log(f'模型 {i+1}/5 完成，AUC={auc:.4f}')
        gc.collect()
    ...
    # 评估集成
    ensemble_config = evaluate_ensemble(temp_dir, model_paths)
