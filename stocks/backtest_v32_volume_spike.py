#!/usr/bin/env python3
"""
选股系统 v3.2 回测 - 成交量放大 (>2 倍) 卖出

基于全市场 5493 只股票回测验证

卖出策略:
- 成交量 > 2 倍 20 日均量 (胜率最高 68.9%)

用法:
    python3 backtest_v32_volume_spike.py --full     # 全市场回测
    python3 backtest_v32_volume_spike.py --sample 500  # 抽样回测
"""
...
class V32VolumeSpikeBacktester:
    """v3.2 成交量放大卖出回测器"""
    ...
    def backtest(self, data: List[Dict]) -> Dict:
        """
        回测策略 (MA15/MA20 金叉买入，成交量放大>2 倍卖出)
            ...
            if not holding:
                ...
                if avg_volume > 0 and current_volume > avg_volume * self.volume_spike_threshold:
                    ...
                    trades.append({
                        ...
                        'price': sell_price,
                        'profit': profit,
                        'reason': f'成交量放大 ({current_volume/avg_volume:.2f}倍)',
                    })
                    shares = 0
                    holding = False
    ...
    def validate_all_stocks(self, sample_size: int = None) -> Dict:
        ...
        """
        print('='*70)
        print('选股系统 v3.2 回测 (成交量放大>2 倍卖出)')
        print('='*70)
        print()
        ...
        if results:
            ...
            # 保存结果
            output_file = self.results_dir / f'v32_volume_spike_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='选股系统 v3.2 回测 (成交量放大>2 倍卖出)')
    parser.add_argument('--sample', type=int, help='抽样数量')
    parser.add_argument('--full', action='store_true', help='全量回测')
    
    args = parser.parse_args()
    
    backtester = V32VolumeSpikeBacktester()
