...
# -*- coding: utf-8 -*-
"""
选股系统 V3.0 回测 - 使用本地缓存数据
基于 V3.0 选股策略，使用 data_tushare 缓存的历史数据
...
用法:
    python backtest_v3_cache.py 000001  # 单只股票回测
    python backtest_v3_cache.py --all --sample 500  # 全市场抽样回测
"""
...
class V3Strategy:
    ...
    def check_filters(self, data, idx):
        ...
        avg_volume = float(data['volume'].iloc[idx-20:idx].mean())
...
class BacktestEngine:
    ...
    def run_backtest(self, strategy, data, print_log=True):
        if print_log:
            ...
            print(f"   可回测天数：{len(data) - strategy.long_ma - 5}")
            print("-" * 70)
...
def backtest_single_stock(symbol: str, args):
    """回测单只股票"""
    print(f"\n{'='*70}")
    print(f"回测股票：{symbol}")
    print(f"{'='*70}")
    ...
    if data is None:
        print(f"❌ {symbol} 数据不存在或加载失败")
        return None
    
    if len(data) < 50:
        print(f"❌ {symbol} 数据不足 50 条")
        return None
    ...
    report = engine.run_backtest(strategy, data, print_log=not args.no_log)
    
    if args.export:
        results_dir = Path(__file__).parent / 'backtest_results'
        results_dir.mkdir(exist_ok=True)
        engine.export_trades(results_dir / f'backtest_v3_{symbol}_trades.csv')
        engine.export_equity_curve(results_dir / f'backtest_v3_{symbol}_curve.csv')
...
def backtest_all_stocks(args):
    ...
    for i, symbol in enumerate(stock_files):
        try:
            ...
            report = engine.run_backtest(strategy, data, print_log=False)
            
            if 'return_metrics' in report:
                results.append({
                    'symbol': symbol,
                    'total_return': report['return_metrics']['total_return'],
                    'annual_return': report['return_metrics']['annual_return'],
                    'trade_count': report['trade_metrics']['trade_count'],
...
回测统计:
   回测股票数：  {total} 只
   盈利股票数：  {profitable} 只 ({profitable/total*100:.1f}%)
   亏损股票数：  {total - profitable} 只
...
收益统计:
   ...
   平均年化：    {statistics.mean(annual_returns)*100:.2f}%
...
风险统计:
   平均最大回撤：{statistics.mean([r['max_drawdown'] for r in results])*100:.2f}%
...
""")
        ...
        output_file = results_dir / f'backtest_v3_all_{timestamp}.csv'
...
def main():
    parser = argparse.ArgumentParser(
        description='选股系统 V3.0 回测 (使用缓存数据)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 单只股票回测
  python backtest_v3_cache.py 000001
  
  # 禁用五重过滤 (推荐！简单策略收益更高)
  python backtest_v3_cache.py 000001 --no-filters
  
  # 全市场回测
  python backtest_v3_cache.py --all
  
  # 抽样回测 500 只
  python backtest_v3_cache.py --all --sample 500
  
  # 导出结果
  python backtest_v3_cache.py 000001 --export
        """
    )
    
    parser.add_argument('stock_code', nargs='?', help='股票代码 (如：000001, 600519)')
    ...
    print(f"✅ 缓存股票数量：{stock_count} 只")
    
    if args.all:
        backtest_all_stocks(args)
    elif args.stock_code:
        backtest_single_stock(args.stock_code, args)
    else:
        print("❌ 请提供股票代码或使用 --all 进行全市场回测")
        print("💡 用法：python backtest_v3_cache.py 000001")
        print("💡 用法：python backtest_v3_cache.py --all --sample 500")
