...
买入信号:
- MA5 上穿 MA10 且 MA10 上穿 MA20 (三均线金叉)
- 或 MA5 同时上穿 MA10 和 MA20
...
用法:
    python3 backtest triple_ma.py --full     # 全市场回测
    python3 backtest triple_ma.py --sample 500  # 抽样回测
"""
...
class TripleMABacktester:
    ...
    def backtest(self, data: List[Dict], trailing_stop_ratio: float) -> Dict:
        """
        回测三均线金叉 + 移动止盈策略
        ...
        for i in range(30, len(data)):
            ...
            ma10 = self.calc_ma(data, 10, i)
            ma20 = self.calc_ma(data, 20, i)
            ma5_prev = self.calc_ma(data, 5, i-1)
            ma10_prev = self.calc_ma(data, 10, i-1)
            ...
            if not holding:
                # 三均线金叉买入
                # 条件 1: MA5 上穿 MA10
                # 条件 2: MA10 上穿 MA20 (或已经在 MA20 之上)
                condition1 = ma5_prev <= ma10_prev and ma5 > ma10
                condition2 = ma10_prev <= ma20_prev and ma10 > ma20
                condition3 = ma10 > ma20  # MA10 已经在 MA20 之上
    ...
    def validate_all_stocks(self, sample_size: int = None) -> Dict:
        ...
        print(f'测试移动止盈：{[ratio_names[r] for r in self.test_ratios]}')
        print()
        ...
        for i, symbol in enumerate(stock_files):
            try:
                ...
                for ratio in self.test_ratios:
                    result = self.backtest(data, ratio)
        ...
        # 保存结果
        output_file = self.results_dir / f'triple_ma_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='三均线金叉 + 移动止盈 策略回测')
