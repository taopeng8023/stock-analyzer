...
# -*- coding: utf-8 -*-
"""
使用 Tushare 补充全市场股票到 2026-04-07 数据
...
print("=" * 80)
print("Tushare 数据补充 - 更新到今日 (2026-04-07)")
print("=" * 80)
...
for i, stock in enumerate(need_update):
    ...
    # 频率限制
    now = time.time()
    if now - last_request < 0.6:  # 加快到 0.6 秒
        time.sleep(0.6 - (now - last_request))
    ...
    try:
        ...
        if filepath.exists():
            ...
            if new_record['日期'] not in existing_dates:
        ...
        if (i + 1) % 500 == 0:
            ...
            print(f"   进度：{i+1}/{len(need_update)} (成功:{success} 已存在:{exists} 无数据:{no_data} 失败:{fail}) 耗时:{elapsed:.0f}s")
...
report_file = LOG_DIR / f'update_report_{TODAY}_final.json'
...
for f in DATA_DIR.glob('*.json'):
    with open(f, 'r', encoding='utf-8') as fp:
        data = json.load(fp)
    if data and data[-1]['日期'] == TODAY:
        today_count += 1

print(f"   今日 ({TODAY}) 数据：{today_count} 只")
print()
print("✅ 数据补充完成！")
