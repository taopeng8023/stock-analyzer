...
print("="*70, flush=True)
print("神经网络全量训练 - 极低学习率版本", flush=True)
print("="*70, flush=True)

files = sorted(HISTORY_DIR.glob("*.json"))
total_files = len(files)
print(f"总股票数: {total_files}", flush=True)
print(f"极低学习率: 128-64-32, LR=0.001, 每批迭代15次", flush=True)
...
model = MLPClassifier(
    ...
    activation='relu',
    solver='adam',
    learning_rate_init=0.001,  # ✅ 极低学习率
    max_iter=15,  # ✅ 每批迭代15次
    warm_start=True,
    random_state=42,
    alpha=0.0001,  # ✅ 极低正则化
...
model_data = {
    ...
    'metadata': {
        ...
        'learning_rate': 0.001, 'max_iter': 15
    }
}

for p in [MODEL_DIR / f"ml_nn_full_low_lr_{ts}.pkl", MODEL_DIR / "ml_nn_production.pkl"]:
    with open(p, 'wb') as f: pickle.dump(model_data, f)

print(f"\n保存: ml_nn_full_low_lr_{ts}.pkl", flush=True)
print("="*70, flush=True)