...
LOG_FILE = Path('/Users/taopeng/.openclaw/workspace/stocks/ml_train_neural_v5.log')
...
def load_and_prepare_stock(stock_file, min_records=150):
    ...
    try:
        ...
        # ===== V5 新增特征 (6 个) =====
        # 1. RSI 变化率
        df['rsi_change'] = df['rsi'].pct_change(3).clip(-10, 10)
        ...
        # 4. 成交量趋势
        df['vol_trend'] = (df['vol_ma5'] / (df['vol'].rolling(20).mean() + 1e-10)).clip(0.1, 10)
        ...
        # 6. 涨跌幅加速度
        df['acc'] = df['pct_chg'].pct_change(3).clip(-10, 10)
        ...
        feature_cols = ['p_ma5', 'p_ma10', 'p_ma20', 'ma5_slope', 'ma10_slope', 
                       ...
                       'rsi_change', 'momentum10', 'vol_change', 'vol_trend', 'price_position', 'acc']
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f'训练日志 V5 - {datetime.now()}\n\n')
    
    log('='*60)
    log('神经网络选股模型 V5 - 深度增强版')
    log('='*60)
    ...
    model = MLPClassifier(
        hidden_layer_sizes=(256, 128, 64, 32),  # 四层网络
        activation='relu',
        solver='adam',
        alpha=0.0005,  # L2 正则化
        batch_size=256,
    ...
    model_path = MODEL_DIR / f'ml_nn_v5_{timestamp}.pkl'
