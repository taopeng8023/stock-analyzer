...
print('='*90)
print('立讯精密 (002475) - 多策略回测分析 v2.0')
print('='*90)
    ...
    for i in range(35, len(klines)):
        ...
        if not holding and macd_prev <= signal_prev and macd > signal:
            ...
            if shares > 0:
                ...
                capital -= cost
                holding = True
                trades.append({'date': klines[i]['day'], 'type': '买入', 'price': buy_price, 'shares': shares, 'signal': 'MACD 金叉'})
        ...
        # MACD 死叉卖出
            ...
            capital += revenue
            profit = ((sell_price - buy_price) / buy_price) * 100
            trades.append({'date': klines[i]['day'], 'type': '卖出', 'price': sell_price, 'shares': shares, 'profit': profit, 'signal': 'MACD 死叉'})
            shares = 0
            holding = False
    ...
    for i in range(15, len(klines)):
        ...
        if not holding and rsi < RSI_OVERSOLD:
            ...
            if shares > 0:
                ...
                capital -= cost
                holding = True
                trades.append({'date': klines[i]['day'], 'type': '买入', 'price': buy_price, 'shares': shares, 'signal': f'RSI={rsi:.1f}'})
        ...
        # RSI 超买卖出
            ...
            capital += revenue
            profit = ((sell_price - buy_price) / buy_price) * 100
            trades.append({'date': klines[i]['day'], 'type': '卖出', 'price': sell_price, 'shares': shares, 'profit': profit, 'signal': f'RSI={rsi:.1f}'})
            shares = 0
            holding = False
    ...
    for i in range(20, len(klines)):
        ...
        if not holding and close <= lower:
            ...
            if shares > 0:
                ...
                capital -= cost
                holding = True
                trades.append({'date': klines[i]['day'], 'type': '买入', 'price': buy_price, 'shares': shares, 'signal': '触及下轨'})
        ...
        # 触及上轨卖出
            ...
            capital += revenue
            profit = ((sell_price - buy_price) / buy_price) * 100
            trades.append({'date': klines[i]['day'], 'type': '卖出', 'price': sell_price, 'shares': shares, 'profit': profit, 'signal': '触及上轨'})
            shares = 0
            holding = False
    ...
    for i in range(20, len(klines)):
        ...
        if not holding and volume_ratio > 2.0 and change > 3.0:
            ...
            if shares > 0:
                ...
                holding = True
                trades.append({'date': klines[i]['day'], 'type': '买入', 'price': buy_price, 'shares': shares, 
                             'signal': f'放量{volume_ratio:.1f}倍 涨{change:.1f}%'})
        
        # 缩量下跌卖出 (持有超过 5 天且跌幅>5%)
        elif holding and i > 5:
            hold_days = 0
            ...
            if change < -5.0:
                ...
                capital += revenue
                profit = ((sell_price - buy_price) / buy_price) * 100
                trades.append({'date': klines[i]['day'], 'type': '卖出', 'price': sell_price, 'shares': shares, 
                             'profit': profit, 'signal': f'跌{change:.1f}%'})
                shares = 0
                holding = False
...
def strategy_turtle():
    ...
    for i in range(N, len(klines)):
        ...
        if not holding and close > high_N:
            ...
            risk_per_trade = capital * 0.01  # 每笔交易风险 1%
        ...
        # 跌破 N 日低点卖出
            ...
            profit = ((sell_price - buy_price) / buy_price) * 100
            trades.append({'date': klines[i]['day'], 'type': '卖出', 'price': sell_price, 'shares': shares,
                         'profit': profit, 'signal': f'跌破{N}日低点{low_N:.2f}'})
            shares = 0
            holding = False
