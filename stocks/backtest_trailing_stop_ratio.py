...
用法:
    python3 backtest_trailing_stop_ratio.py --full     # 全市场回测
    python3 backtest_trailing_stop_ratio.py --sample 500  # 抽样回测
"""
...
class TrailingStopRatioBacktester:
    """移动止盈比例回测器"""
    ...
    def __init__(self, cache_dir: str = None):
        ...
        # 测试的移动止盈比例
        self.test_ratios = [0.05, 0.10, 0.15, 0.20]
    ...
    def backtest_with_ratio(self, data: List[Dict], trailing_stop_ratio: float) -> Dict:
        """
        回测特定移动止盈比例
        
        Args:
            data: 股票数据
            trailing_stop_ratio: 移动止盈比例 (0.05=5%, 0.10=10%, etc)
            ...
            if not holding:
                ...
                if current_price < trailing_stop:
                    ...
                    trades.append({
                        ...
                        'price': sell_price,
                        'profit': profit,
                        'reason': f'移动止盈 ({trailing_stop_ratio*100:.0f}%)',
                    })
                    shares = 0
                    holding = False
        ...
        for i, t in enumerate(trades):
            if t['type'] == '买入':
                ...
                sell_p = t['price']
                # 简化：假设最高价是买入价和卖出价的较大者
                highest_p = max(buy_p, sell_p) * 1.1  # 假设最高价比卖出价高 10%
                optimal_profit = (highest_p - buy_p) / buy_p * 100
                actual_profit = (sell_p - buy_p) / buy_p * 100
                if optimal_profit > 0:
                    capture_ratio = actual_profit / optimal_profit * 100
                    capture_ratios.append(capture_ratio)
        
        avg_capture_ratio = statistics.mean(capture_ratios) if capture_ratios else 0
    ...
    def validate_all_stocks(self, sample_size: int = None) -> Dict:
        ...
        print(f'测试比例：{[f"{int(r*100)}%" for r in self.test_ratios]}')
        print()
        ...
        for i, symbol in enumerate(stock_files):
            try:
                ...
                for ratio in self.test_ratios:
                    result = self.backtest_with_ratio(data, ratio)
        ...
        print(f'{"比例":<10} {"股票数":>10} {"交易数":>10} {"平均收益":>12} {"胜率":>10} {"持仓天":>10} {"捕获率":>10}')
        print('-'*90)
        ...
        for ratio in self.test_ratios:
            ...
            avg_capture = statistics.mean(r['avg_capture_ratio'] for r in results)
            ...
            print(f'{ratio_str:<10} {stock_count:>10} {total_trades:>10} {avg_return:>+11.2f}% {avg_win_rate:>9.1f}% {avg_hold_days:>10.1f} {avg_capture:>9.1f}%')
        ...
        if summary['ratios']:
            ...
            print()
            print(f'🏆 最佳比例：{int(best_ratio*100)}%')
            ...
            print(f'   收益捕获率：{summary["ratios"][best_ratio]["avg_capture_ratio"]:.1f}%')
        
        # 保存结果
        output_file = self.results_dir / f'trailing_stop_ratio_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
