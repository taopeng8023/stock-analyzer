...
for fp in files:
    ...
    print(f"  2024年范围: {idx_start} - {idx_end}")
    ...
    print(f"  有效范围: {valid_start} - {valid_end}")
...
# 保存
model_file = MODEL_DIR / f"ml_nn_v8_test_{datetime.now().strftime('%Y%m%d_%H%M')}.pkl"
with open(model_file, 'wb') as f:
    pickle.dump({
        'model': m,
        'scaler': scaler,
        'features': FEATURES,
        'metadata': {'auc': auc, 'precision_90': p90}
    }, f)
