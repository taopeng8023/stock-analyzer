#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
使用 Tushare 补充全市场历史行情数据

从最后一条数据更新到今天 (2026-04-07)
"""

import json
import time
import requests
from pathlib import Path
from datetime import datetime, timedelta

# 配置
TUSHARE_TOKEN = 'a7358a0255666758b3ef492a6c1de79f2447de968d4a1785b73716a4'
DATA_DIR = Path('/home/admin/.openclaw/workspace/stocks/data_tushare')
LOG_DIR = Path('/home/admin/.openclaw/workspace/stocks/logs')

# 目标日期
TODAY = '20260407'
START_DATE = '20260328'  # 从 3 月 28 日开始补充

# API
api_url = 'http://api.tushare.pro'

print("=" * 80)
print("Tushare 数据补充更新")
print("=" * 80)
print(f"Token: {TUSHARE_TOKEN[:20]}...")
print(f"补充日期：{START_DATE} ~ {TODAY}")
print(f"数据目录：{DATA_DIR}")
print()

# 1. 获取股票列表
print("[1/4] 获取股票列表...")
stock_payload = {
    'api_name': 'stock_basic',
    'token': TUSHARE_TOKEN,
    'params': {'list_status': 'L'},
    'fields': ''
}

try:
    resp = requests.post(api_url, json=stock_payload, timeout=60)
    result = resp.json()
    
    if result.get('code') != 0:
        print(f"❌ 获取股票列表失败：{result.get('msg')}")
        exit(1)
    
    data = result.get('data')
    if data and 'items' in data:
        fields = data.get('fields', [])
        items = data.get('items', [])
        
        stocks = []
        for item in items:
            record = dict(zip(fields, item))
            ts_code = record.get('ts_code', '')
            if ts_code:
                stocks.append({
                    'ts_code': ts_code,
                    'symbol': ts_code.split('.')[0]
                })
        
        print(f"   股票总数：{len(stocks)} 只")
    else:
        print("❌ 未获取到股票列表")
        exit(1)
        
except Exception as e:
    print(f"❌ 异常：{e}")
    exit(1)

# 2. 检查现有数据
print(f"\n[2/4] 检查现有数据...")

need_update = []
skip = 0

for stock in stocks:
    symbol = stock['symbol']
    filepath = DATA_DIR / f'{symbol}.json'
    
    if filepath.exists():
        with open(filepath, 'r', encoding='utf-8') as f:
            data = json.load(f)
        
        if data:
            latest_date = data[-1]['日期']
            
            # 如果最新日期早于今日，需要更新
            if latest_date < TODAY:
                need_update.append({
                    'symbol': symbol,
                    'ts_code': stock['ts_code'],
                    'latest_date': latest_date
                })
            else:
                skip += 1
    else:
        need_update.append({
            'symbol': symbol,
            'ts_code': stock['ts_code'],
            'latest_date': None
        })

print(f"   需要更新：{len(need_update)} 只")
print(f"   已是最新：{skip} 只")

# 3. 批量获取数据
print(f"\n[3/4] 获取 {START_DATE} ~ {TODAY} 行情数据...")
print("   预计耗时：20-40 分钟")
print()

success = 0
exists = 0
fail = 0
no_data = 0
last_request = 0

start_time = time.time()

for i, stock in enumerate(need_update):
    ts_code = stock['ts_code']
    symbol = stock['symbol']
    filepath = DATA_DIR / f'{symbol}.json'
    
    # 频率限制
    now = time.time()
    if now - last_request < 0.8:  # 加快到 0.8 秒
        time.sleep(0.8 - (now - last_request))
    last_request = time.time()
    
    # 获取数据
    daily_payload = {
        'api_name': 'daily',
        'token': TUSHARE_TOKEN,
        'params': {
            'ts_code': ts_code,
            'start_date': START_DATE,
            'end_date': TODAY
        },
        'fields': ''
    }
    
    try:
        resp = requests.post(api_url, json=daily_payload, timeout=30)
        result = resp.json()
        
        if result.get('code') != 0:
            fail += 1
            continue
        
        data = result.get('data')
        if not data or 'items' not in data or not data['items']:
            no_data += 1
            continue
        
        # 解析数据
        fields = data.get('fields', [])
        items = data.get('items', [])
        
        new_records = []
        for item in items:
            record = dict(zip(fields, item))
            new_records.append({
                '日期': str(record.get('trade_date', '')),
                '开盘': float(record.get('open', 0)),
                '收盘': float(record.get('close', 0)),
                '最高': float(record.get('high', 0)),
                '最低': float(record.get('low', 0)),
                '成交量': float(record.get('vol', 0)) * 100,
                '成交额': float(record.get('amount', 0)) * 1000,
                '涨跌幅': float(record.get('pct_chg', 0))
            })
        
        # 读取现有数据
        if filepath.exists():
            with open(filepath, 'r', encoding='utf-8') as f:
                existing_data = json.load(f)
            
            # 合并数据（去重）
            existing_dates = {d['日期'] for d in existing_data}
            
            for rec in new_records:
                if rec['日期'] not in existing_dates:
                    existing_data.append(rec)
                    existing_dates.add(rec['日期'])
            
            # 排序
            existing_data.sort(key=lambda x: x['日期'])
            
            # 保存
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(existing_data, f, ensure_ascii=False, indent=2)
        else:
            # 新文件
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(new_records, f, ensure_ascii=False, indent=2)
        
        success += 1
        
        # 进度
        if (i + 1) % 500 == 0:
            elapsed = time.time() - start_time
            print(f"   进度：{i+1}/{len(need_update)} (成功:{success} 无数据:{no_data} 失败:{fail}) 耗时:{elapsed:.0f}s")
    
    except Exception as e:
        fail += 1

elapsed = time.time() - start_time

# 4. 完成统计
print()
print("=" * 80)
print("执行完成")
print("=" * 80)
print(f"   需要更新：{len(need_update)} 只")
print(f"   成功更新：{success} 只")
print(f"   无数据：{no_data} 只")
print(f"   失败：{fail} 只")
print(f"   已是最新：{skip} 只")
print(f"   总耗时：{elapsed:.1f}秒 ({elapsed/60:.1f}分钟)")
print("=" * 80)

# 保存报告
report = {
    'date': datetime.now().isoformat(),
    'period': f'{START_DATE} ~ {TODAY}',
    'summary': {
        'total_stocks': len(stocks),
        'need_update': len(need_update),
        'success': success,
        'no_data': no_data,
        'fail': fail,
        'already_latest': skip
    },
    'elapsed': elapsed
}

report_file = LOG_DIR / f'update_report_{TODAY}.json'
with open(report_file, 'w', encoding='utf-8') as f:
    json.dump(report, f, indent=2, ensure_ascii=False)

print(f"\n💾 报告已保存：{report_file}")

# 验证
print("\n验证数据...")
test_files = ['000001.json', '600519.json', '300750.json']
for name in test_files:
    f = DATA_DIR / name
    if f.exists():
        with open(f, 'r', encoding='utf-8') as fp:
            data = json.load(fp)
        print(f"   {name}: {len(data)}条，最新：{data[-1]['日期']}")

print()
print("✅ 数据补充完成！")
