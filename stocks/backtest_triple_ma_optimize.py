...
用法:
    python3 backtest_triple_ma_optimize.py --full     # 全市场回测
    python3 backtest_triple_ma_optimize.py --sample 500  # 抽样回测
"""
...
class TripleMAOptimizeBacktester:
    """三均线配置优化回测器"""
    ...
    def backtest(self, data: List[Dict], ma_config: Tuple[int, int, int], trailing_stop_ratio: float) -> Dict:
        """
        回测三均线金叉 + 移动止盈策略
        
        Args:
            data: 股票数据
            ma_config: 三均线配置 (短，中，长)
            trailing_stop_ratio: 移动止盈比例
        
        Returns:
            回测结果
        """
        ma_short, ma_mid, ma_long = ma_config
        ...
        for i in range(ma_long, len(data)):
            ...
            ma_m = self.calc_ma(data, ma_mid, i)
            ma_l = self.calc_ma(data, ma_long, i)
            ma_s_prev = self.calc_ma(data, ma_short, i-1)
            ma_m_prev = self.calc_ma(data, ma_mid, i-1)
            ma_l_prev = self.calc_ma(data, ma_long, i-1)
            
            if None in (ma_s, ma_m, ma_l, ma_s_prev, ma_m_prev, ma_l_prev):
                continue
            
            if not holding:
                # 三均线金叉买入
                # 条件 1: 短期均线上穿中期均线
                # 条件 2: 中期均线上穿长期均线 (或已经在长期均线之上)
                condition1 = ma_s_prev <= ma_m_prev and ma_s > ma_m
                condition2 = ma_m_prev <= ma_l_prev and ma_m > ma_l
                condition3 = ma_m > ma_l  # 中期均线已经在长期均线之上
                ...
                if condition1 and (condition2 or condition3):
                    ...
                    if shares > 0:
                        ...
                        trades.append({
                            ...
                            'type': '买入',
                            'price': buy_price,
                            'config': f'{ma_short}/{ma_mid}/{ma_long}',
                        })
            else:
                # 更新最高价
    ...
    def validate_all_stocks(self, sample_size: int = None) -> Dict:
        ...
        for config in self.test_configs:
            config_name = config_names.get(config, f'{config[0]}/{config[1]}/{config[2]}')
        ...
        # 保存结果
        output_file = self.results_dir / f'triple_ma_optimize_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
