...
# -*- coding: utf-8 -*-
"""
神经网络选股模型 V7 - 精简版 (内存优化)

优化方向:
- 股票数量：1500 → 1000 只
- 样本数量：80 万 → 60 万
- 网络规模：256-128-64 → 128-64-32
- 标签：涨超 5% (保持高收益目标)
"""
...
LOG_FILE = Path('/Users/taopeng/.openclaw/workspace/stocks/ml_train_neural_v7_lite.log')
...
def prepare_data():
    ...
    # 精简：1000 只股票
    n_stocks = min(1000, len(stock_files))
    ...
    # 精简：60 万样本
    n_pos = min(len(positives), 150000)
    n_neg = min(len(negatives), 450000)
    ...
    # 保存临时数据
    temp_dir = MODEL_DIR / 'temp_v7_lite'
...
def train_single_model(temp_dir, seed, model_idx):
    ...
    # 保存单个模型
    model_path = MODEL_DIR / f'ml_nn_v7_lite_model{model_idx+1}_seed{seed}.pkl'
...
def evaluate_ensemble(temp_dir, model_paths):
    ...
    config_path = MODEL_DIR / f'ml_nn_v7_lite_ensemble_config_{timestamp}.json'
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f'训练日志 V7 精简版 - {datetime.now()}\n\n')
    ...
    for i, seed in enumerate(seeds):
        model_path = MODEL_DIR / f'ml_nn_v7_lite_model{i+1}_seed{seed}.pkl'
