...
def merge_batch_results(all_batch_results):
    ...
    return all_returns, all_probs, all_stock_stats

def analyze_results(all_returns, all_probs, all_stock_stats, threshold):
    """分析回测结果"""
...
def run_batch_backtest(n_stocks=500, batch_size=150, threshold=0.90):
    """运行分批回测"""
    log('='*60)
    log('V8 集成模型分批回测')
    log('='*60)
    ...
    for batch_idx in range(n_batches):
        ...
        batch_file = RESULTS_DIR / f'backtest_v8_batch{batch_idx+1}_{timestamp}.json'
        ...
        log(f'  批次结果：{batch_file.name}')
        
        # 强制垃圾回收
        del returns, probs, stock_stats
        gc.collect()
    ...
    all_returns, all_probs, all_stock_stats = merge_batch_results(all_batch_results)
    
    # 分析最终结果
    stats = analyze_results(all_returns, all_probs, all_stock_stats, threshold)
    ...
    result_file = RESULTS_DIR / f'backtest_v8_ensemble_final_{timestamp}.json'
    ...
    log(f'{"V8 组合版":<12} | {stats["expected_return"]*100:>9.2f}% | {stats["win_rate"]*100:>7.1f}% | {stats["total_trades"]:>8}')
    
    if v7_results['expected_return'] > 0:
        improvement = (stats['expected_return'] - v7_results['expected_return']) / v7_results['expected_return'] * 100
        log(f'\n相比 V7: 期望收益 {improvement:+.1f}%')
    
    return final_results

if __name__ == '__main__':
    results = run_batch_backtest(n_stocks=500, batch_size=150, threshold=0.90)
