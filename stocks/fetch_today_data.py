...
try:
    ...
    if result.get('code') != 0:
        print(f"❌ Token 无效：{result.get('msg')}")
        print("\n请检查:")
        print("1. Token 是否正确")
        print("2. Tushare 积分是否足够")
        print("3. 访问 https://tushare.pro/user/token 查看 Token 状态")
        exit(1)
...
for i, stock in enumerate(stocks):
    ...
    try:
        ...
        # 进度
        if (i + 1) % 500 == 0:
            print(f"   进度：{i+1}/{len(stocks)} (成功:{success} 已存在:{exists} 无数据:{no_data} 失败:{fail})")
