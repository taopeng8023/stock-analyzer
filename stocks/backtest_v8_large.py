...
def run_large_backtest(n_stocks=2000, batch_size=200, threshold=0.90):
    """运行大规模回测"""
    log('='*60)
    log('V8 集成模型大规模回测')
    log('='*60)
    ...
    for batch_idx in range(n_batches):
        ...
        batch_file = RESULTS_DIR / f'backtest_v8_large_batch{batch_idx+1}_{timestamp}.json'
    ...
    result_file = RESULTS_DIR / f'backtest_v8_large_{n_stocks}stocks_{timestamp}.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-stocks', type=int, default=2000, help='回测股票数量')
    parser.add_argument('--batch-size', type=int, default=200, help='每批股票数')
