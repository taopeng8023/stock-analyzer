...
优化方向:
- 数据量：1000 股 → 1500 股 (+50%)
- 网络规模：128-64-32 → 256-128-64 (翻倍)
- 新增特征：20 维 → 28 维 (资金流/量价/动量衍生)
- 标签：涨超 5% (保持)
"""
...
LOG_FILE = Path('/Users/taopeng/.openclaw/workspace/stocks/ml_train_neural_v8.log')
...
def prepare_data():
    ...
    # V8: 70 万样本 (增加但控制内存)
    n_pos = min(len(positives), 175000)
    n_neg = min(len(negatives), 525000)
    ...
    log(f'特征矩阵：{X.shape}')
    log(f'特征维度：{X.shape[1]} (V7: 20 维 → V8: 28 维)')
...
def train_single_model(temp_dir, seed, model_idx):
    ...
    # V8: 256-128-64 (vs V7 的 128-64-32)
    model = MLPClassifier(
    ...
    # 保存单个模型
    model_path = MODEL_DIR / f'ml_nn_v8_model{model_idx+1}_seed{seed}.pkl'
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f'训练日志 V8 - {datetime.now()}\n\n')
    ...
    for i, seed in enumerate(seeds):
        model_path = MODEL_DIR / f'ml_nn_v8_model{i+1}_seed{seed}.pkl'
