...
print("="*70, flush=True)
print("神经网络训练 - 2000股超优化版", flush=True)
print("="*70, flush=True)
...
files = sorted(HISTORY_DIR.glob("*.json"))[:2000]
...
# 超激进参数：最小网络 + 最少迭代 + 最高学习率
model = MLPClassifier(
    hidden_layer_sizes=(32, 16),  # 最小网络
    activation='relu',
    solver='adam',
    learning_rate_init=0.05,  # 最高学习率
    max_iter=5,  # 最少迭代
    early_stopping=True,
    validation_fraction=0.05,  # 减少验证比例
    n_iter_no_change=1,
    random_state=42,
    alpha=0.01
)
...
model_data = {
    ...
    'metadata': {
        'auc': auc, 'version': 'V10_2000股_超优',
        'train_samples': len(X_train), 'stocks': stocks_ok,
        'load_time': load_time, 'train_time': train_time, 'total_time': total_time,
        'network': '32-16', 'max_iter': 5
    }
}

for p in [MODEL_DIR / f"ml_nn_2000_{ts}.pkl", MODEL_DIR / "ml_nn_production.pkl"]:
    with open(p, 'wb') as f: pickle.dump(model_data, f)

print(f"\n保存: ml_nn_2000_{ts}.pkl", flush=True)
print("="*70, flush=True)