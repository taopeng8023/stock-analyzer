...
"""
全量训练 - 折中优化版本
网络64-32（已验证成功），迭代10次，学习率0.005
目标：在已成功网络基础上增加迭代次数提升精确率
"""
...
print("="*70, flush=True)
print("神经网络全量训练 - 折中优化版本", flush=True)
print("="*70, flush=True)

files = sorted(HISTORY_DIR.glob("*.json"))
total_files = len(files)
print(f"总股票数: {total_files}", flush=True)
print(f"折中优化: 网络64-32, 学习率0.005, 每批迭代10次", flush=True)

# ✅ 折中优化：已验证成功的64-32网络 + 更低学习率 + 更多迭代
model = MLPClassifier(
    hidden_layer_sizes=(64, 32),  # ✅ 已验证成功
    activation='relu',
    solver='adam',
    learning_rate_init=0.005,  # ✅ 更低学习率
    max_iter=10,  # ✅ 增加到10次迭代
    warm_start=True,
    random_state=42,
    alpha=0.001,  # ✅ 中等正则化
...
model_data = {
    ...
    'metadata': {
        'auc': auc, 'version': 'V10_全量折中优化',
        'total_stocks': total_stocks, 'total_samples': total_samples,
        'test_samples': len(test_y_all), 'total_time': total_time,
        'batches': batch_results, 'network': '64-32',
        'learning_rate': 0.005, 'max_iter': 10
    }
}

for p in [MODEL_DIR / f"ml_nn_full_balanced_{ts}.pkl", MODEL_DIR / "ml_nn_production.pkl"]:
    with open(p, 'wb') as f: pickle.dump(model_data, f)

print(f"\n保存: ml_nn_full_balanced_{ts}.pkl", flush=True)
print("="*70, flush=True)