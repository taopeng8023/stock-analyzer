...
# -*- coding: utf-8 -*-
"""
使用东方财富 API 获取今日收盘后交易数据（简化版）

直接获取全市场今日行情，无需先获取股票列表
"""
...
print(f"现有数据文件：{len(existing_files)} 只")
print()

# 东方财富批量接口
def get_market_quotes(market, page=1, page_size=500):
    """
    获取市场批量行情
...
def main():
    ...
    # 获取沪市和深市数据
    for market_name, market_id in [('沪市', 1), ('深市', 0)]:
        print(f"获取 {market_name} 数据...")
        
        page = 1
        while True:
            quotes = get_market_quotes(market_id, page, 500)
            
            if not quotes:
                break
            
            print(f"  页码 {page}: {len(quotes)} 只股票")
